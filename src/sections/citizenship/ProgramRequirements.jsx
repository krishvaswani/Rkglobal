      ...
      <div className="max-w-[1400px] mx-auto flex flex-col">
        <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4 md:gap-10 mb-6 md:mb-12">
          <div className="flex-1">
            <h2 className="text-[24px] md:text-[36px] lg:text-[40px] font-extrabold text-[#111] uppercase tracking-tight leading-[1.1]">
              {`Requirements for ${program.name}`}
            </h2>
          </div>
          <div className="flex-1 lg:text-right">
            <p className="text-[12px] md:text-sm lg:text-base text-gray-600 max-w-[420px] ml-auto leading-[1.7]">
        ...
        <div className="relative w-full rounded-[10px] md:rounded-[10px] overflow-hidden bg-[#0c2b6b]">
          ...
          <div className="relative z-10 p-4 md:p-8 flex flex-col md:flex-row md:items-center gap-4 md:gap-10">
            <div className="flex-1">
              <p className="text-white text-[15px] md:text-[22px] leading-[1.5] font-medium">
                {program.requirementsSummary}
              </p>
            </div>
              ...
              <ul className="flex flex-col gap-2.5 md:gap-4">
                {program.requirementsChecklist.map((item) => (
                  <li key={item} className="flex items-start gap-3 md:gap-4">
                    <CheckCircle2 color="#0c40e8" size={18} className="mt-0.5 flex-shrink-0" />
                    <span className="text-gray-700 text-[12px] md:text-[14px] font-medium leading-relaxed">
                      {item}
                    </span>
                  </li>
                ))}
